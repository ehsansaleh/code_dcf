...
class distLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # matrix product by forward function, but when using WeightNorm, this also multiply the cosine distance
        # by a class-wise learnable norm, see the issue#4&8 in the github
...
class WideResNet(nn.Module):
    def __init__(self, depth=28, widen_factor=10, num_classes=200,
                 loss_type='dist', per_img_std=False, stride=1):
        dropRate = 0.5
        flatten = True
...
def wrn28_10(num_classes=200, loss_type='dist'):
    model = WideResNet(depth=28, widen_factor=10, num_classes=num_classes,
                       loss_type=loss_type, per_img_std=False, stride=1)
    return model
